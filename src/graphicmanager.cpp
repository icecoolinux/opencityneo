...
{
	DisplayStructure(pcStructure, rcuiW, rcuiL, _uiDisplayListMask);
}
...
// The "switch" instruction will be removed once we have enough models
	switch (enumGC) {
		...
		case OC_TREE_PINE1:
		case OC_TREE_PINE2:
			_tabpModel[enumGC]->DisplayList( rcuiW, rcuiL, tabH, dlMask );
			break;
...
//debug cout << "empty" << endl;
		...
		case OC_TEST_BUILDING:
			if (_tabpModel[OC_TEST_BUILDING] != NULL) {
				_tabpModel[OC_TEST_BUILDING]->DisplayList( rcuiW, rcuiL, tabH, dlMask );
			}
			break;
...
// Set mask for display only the geometry.
	uint dlMaskOnlyGeometry = OC_ONLY_GEOMETRY_LIST;

// Display only the geometry of the structures.
	glPushAttrib( GL_ENABLE_BIT );
	glDisable( GL_TEXTURE_2D );
	glDisable( GL_LIGHTING );

	this->DisplayStructure( pcStructure, rcuiW, rcuiL, dlMaskOnlyGeometry );
