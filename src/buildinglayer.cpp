...
// For more obvious coding
	...
	switch (enumStructCode) {
		...
		case OC_STRUCTURE_PARK:
		case OC_STRUCTURE_TEST:
			errCode = _BuildRCIStructure( W1, L1, W2, L2, enumStructCode, rCost, cityFund );
			break;
		...
		case OC_STRUCTURE_EPLANT_COAL:
		case OC_STRUCTURE_EPLANT_NUCLEAR:
			errCode = _BuildWEGStructure( W1, L1, enumStructCode, rCost, cityFund );
			break;

		case OC_STRUCTURE_ROAD:
		case OC_STRUCTURE_ELINE:
			errCode = _BuildPathStructure( W1, L1, W2, L2, enumStructCode, rCost, cityFund );
			break;

		case OC_STRUCTURE_FLORA:
			errCode = _BuildFloraStructure( W1, L1, W2, L2, enumStructCode, rCost, cityFund );
			break;
...
printf("jaja\n");
	...
	uint uiFund = 10;								// Temp fund, necessary to call internal methods
...
) const
{
	...
	return WindowFactory::createQueryWindow(pstruct);
}
...
//cout << "size of RCIStructure is: " << sizeof( RCIStructure ) << endl;
	...
	for (l = L1; l <= L2; l++) {
		...
		for (w = W1; w <= W2; w++, linearIndex++) {
			if ((_tabpStructure[ linearIndex ] == NULL)
			 && gVars.gpMapMgr->IsSquarePlane(w, l) ) {
				rCost += cost;
			}
		}
	}
...
// Let's do it
	OPENCITY_ERR_CODE errorCode = _BuildWEGStructure( W1, L1, W2, L2, enumStructCode, rCost, cityFund );
