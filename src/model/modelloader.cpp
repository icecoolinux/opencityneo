...
ModelLoader::LoadAC3D( const string & rcsFileName )
{
	...
	GLuint list = 0, listTwoSide = 0, listAlpha = 0, listOnlyGeometry = 0;
...
// Restore all enabled bits
	...
	/*=====================================================================*/
// Recursively load all the objects into the only geometry display list
	listOnlyGeometry = glGenLists( 1 );
	glNewList( listOnlyGeometry, GL_COMPILE );
	glPushAttrib( GL_ENABLE_BIT );

// Load all the vertex
	glBegin( GL_TRIANGLES );
	_AC3DVertexToGL( strPath, vMaterial, pObject, false, false, true);
	glEnd();
...
//		 << " / vertex: " << nbVertex << endl;
	...
	if (glIsTexture(uiName)) {
		pModel = new Model( list, listTwoSide, listAlpha, listOnlyGeometry, strPath + "/" + strFile );
	}
	else {
		pModel = new Model( list, listTwoSide, listAlpha, listOnlyGeometry );
	}
	assert( pModel != NULL );
...
// Process only objects that we are asked to do
	...
	// Process only the polygons with the requested number of sides
		if (!bProcessTranslucent and !bOnlyGeometry and (bProcessTwoSide xor vpSurface[pos]->IsTwoSide())) {
			continue;
		}
...
// Parse all the child objects
	...
	sizeObj = vpObj.size();
	for (posObj = 0; posObj < sizeObj; posObj++) {
		_AC3DVertexToGL( strPath, vMaterial, vpObj[posObj], bProcessTranslucent, bProcessTwoSide, bOnlyGeometry );
	}
