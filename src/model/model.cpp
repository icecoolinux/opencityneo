...
_uiAlpha( dlAlpha ),
_uiOnlyGeometry( dlOnlyGeometry )
{
...
// Delete display lists
	...
	if (glIsList( _uiOnlyGeometry ))
		glDeleteLists( _uiOnlyGeometry, 1 );
}
...
//	glTranslatef( rcfW, tabY[0], rcfL );
	...
	if (dlMask & OC_ALPHA_LIST and glIsList( _uiAlpha ))
		glCallList( _uiAlpha );
	if (dlMask & OC_ONLY_GEOMETRY_LIST and glIsList( _uiOnlyGeometry ))
		glCallList( _uiOnlyGeometry );
	glPopMatrix();
}
