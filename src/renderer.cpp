...
// call the right method with appropriate values
	return GetSelectedWLFrom(
		...
		pcMap,
		pcLayer,
		gVars.guiCityEdge, gVars.guiCityEdge,
		_uiCityWidth-gVars.guiCityEdge-1, _uiCityLength-gVars.guiCityEdge-1
	);
}
...
*/
   ...
   /*=====================================================================*/
	// Old version, kept for reference, may 10th, 2015
/*
const bool
Renderer::GetSelectedWLFrom
(
...
//debug // cout << "Min depth: " << uiDepthMin << endl;
		...
		if( ruiL > rcuiL2 ) ruiL = rcuiL2;
		if( ruiW > rcuiW2 ) ruiW = rcuiW2;
...
// Read pixel clicked.
	glReadPixels(rcuiMouseX, gVars.guiScreenHeight-rcuiMouseY, 1, 1,  GL_RGBA,  GL_UNSIGNED_BYTE,  pixel);

// Calculate id with color code.
	id = pixel[0]*256*256 + pixel[1]*256 + pixel [2];

/* Uncomment for view the color code on click.
SDL_GL_SwapBuffers();
sleep(2);*/
